...
async function generateItinerarioPdf(itinerario, formData, planLabel) {
  try {
    ...
    content.push({ text: 'VIAJ� M�S. PLANIFIC� MENOS.', fontSize: 9, color: '#888', alignment: 'center', margin: [0, 0, 0, 6] });
    ...
    const resumenRows = [
      ...
      formData.dias                         ? ['Duraci�n',             `${formData.dias} d�as`]                          : null,
      formData.numViajeros                  ? ['Viajeros',             String(formData.numViajeros)]                      : null,
      resumen.fecha_optima_texto            ? ['Mejor �poca',          resumen.fecha_optima_texto]                        : null,
    ...
    if (itinerario.dias?.length) {
      ...
      content.push({ text: 'ITINERARIO D�A A D�A', fontSize: 13, bold: true, color: coral, margin: [0, 0, 0, 10] });
      itinerario.dias.forEach(dia => {
        content.push({ text: `D�a ${dia.numero}: ${dia.titulo || ''}`, fontSize: 12, bold: true, color: coral, margin: [0, 0, 0, 5] });
        const rows = [
          dia.manana?.actividad  ? `?? Ma�ana: ${dia.manana.actividad}`     : null,
        ...
        if (dia.gasto_dia) content.push({ text: `?? Gasto del d�a: ${dia.gasto_dia}`, fontSize: 10, color: '#777', italics: true, margin: [10, 0, 0, 2] });
    ...
    if (itinerario.salud_seguridad) {
      ...
      if (ss.emergencias) {
        ...
        const emTxt = [em.policia && `Polic�a: ${em.policia}`, em.ambulancia && `Ambulancia: ${em.ambulancia}`, em.bomberos && `Bomberos: ${em.bomberos}`].filter(Boolean).join(' � ');
    ...
    if (itinerario.que_empacar) {
      content.push(sep());
      content.push({ text: '?? QU� EMPACAR', fontSize: 13, bold: true, color: coral, margin: [0, 0, 0, 8] });
...
function getCountryTravelContext(origenStr, destinoStr) {
  ...
  const isPer�      = o.includes('per�') || o.includes('peru') || o.includes('lima') || o.includes('arequipa') || o.includes('cusco') || o.includes('trujillo') || o.includes('piura') || o.includes('iquitos');
